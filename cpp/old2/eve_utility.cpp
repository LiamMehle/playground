...
// basicaly a 2nd main function for this one tool.
int skill_suggest(const uint32_t target) {
	...
	std::string raw_data;
	{
		...
		raw_data = pbuffer;
		delete[] pbuffer; // invalidate pbuffer and buffer
	}                   // delete     pbuffer and buffer
	...
	std::vector<skill> suggestions;
	suggestions.reserve(2); // a reasonable guess
	skill buffer;
	//constexpr uint32_t day_in_min = 24*60;
	for(auto e : data){
		auto time = e.second;
		if(time > target) {
			buffer.name = e.first;
			buffer.time = time;
			suggestions.emplace_back(buffer);
		} else {
		}
	}
	for(unsigned int i = 0; i<suggestions.size();i++) {
		printf("%s\t%u\n", suggestions[i].name.c_str(), suggestions[i].time);
	}
	// get element or pair of elements that is closest to
	// target where if a pair is selected the first must
	// be at most 24h-1m (alpha rules to limit the training
	// queue)
