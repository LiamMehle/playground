	...
	int init_instance() {
		{
			...
			instance_extensions.emplace_back("VK_KHR_xcb_surface");
		...
		for(int i = 0; i < gpu_list.size(); i++) {
			uint property_count;
			error_check(vkEnumerateDeviceExtensionProperties(gpu_list[i], NULL, &property_count, nullptr));
			std::vector<VkExtensionProperties> property_list(property_count);
			error_check(vkEnumerateDeviceExtensionProperties(gpu_list[i], NULL, &property_count, property_list.data()));
			for(int j = 0; j < property_count; j++) {
				if(property_list[j].extensionName == "VK_KHR_swapchain") {
					gpu = gpu_list[j];
					std::cout << "using gpu " << j << std::endl;
					goto found_gpu;
				}
			}	
		}
		std::cout << "couldn't find suitable gpu, using the first" << std::endl;
		gpu = gpu_list[0];
		found_gpu:
